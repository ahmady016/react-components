...
export default () => {
  ...
  Array.random = function({ count = 10, unique = true, stringOps = null }) {
    let _currString,
        _result = [];
    if (!stringOps) {
      stringOps = {
    ...
    for (let i = 0; i < count; i++) {
      _currString = String.random(stringOps);
      if (unique) {
  ...
  String.random = function({ lower = false, upper = true, numbers = true, symbols = '', len = 8, base = '' }) {
    let _currChar = "",
        _prevChar = "",
        _result = "";
    if (!base) {
      ...
      if(lower)
        base += _allStrings.lower;
      if(upper)
        base += _allStrings.upper;
      if(numbers)
        base += _allStrings.numbers;
      if(symbols)
        base += symbols;
    }
    for(var i=0;i<len;i++) {
      ...
      do {
          _currChar = base.charAt(Number.between(0,base.length-1));
  ...
  Date.prototype.duration = function(format = "d") {
    // each time unit relevant to seconds used to do units calc
    const readableFormat = (val, unit) =>
    ...
    if (format === "d") {
      ...
      for (let unit of Object.keys(time)) {
        if (seconds >= time[unit]) {
          ...
          seconds = seconds % time[unit];
          result.push(
            format === "r" ? readableFormat(val, unit) : shortFormat(val, unit)
          );
        }
      }
      // get the last unit
  ...
  // order Array of Objects by a given object key [ascending - descending]
  Array.prototype.orderBy = function(prop) {
    ...
    if(!prop || this.some(item => typeof item !== 'object'))
      return this.sort();
